...
## Введение (автор: Александр Гребенщиков)

Большие языковые модели очень требовательны к ресурсам во время обучения и использования. Чрезвычайно высокие затраты как по памяти, так и по времени во время инференса, является серьезным препятствием для внедрения трансформеров для решения реальных задач.
...
- *Большой расход памяти*. Как параметры модели, так и промежуточные состояния необходимы в памяти во время инференса. Например,
  - Кэш KV должен храниться в памяти во время стадии декодирования; например, для моделей с 500+ миллиардами параметров, размера батча 512 и длины контекста 2048 общий объем кэша KV составляет 3 ТБ (!).
...
## Квантизация (автор: Роман Столяров)
...
## Дистилляция знаний (Knowledge distillation) (автор: Влад Ушаков)
...
<figure>
  ...
  <figcaption>Визуализация эффекта температуры: так меняется дискретное распределение на пяти исходах с некоторыми логитами при изменении температуры</figcaption>
</figure>

#### Что получилось

Модель-учитель на тестовом множестве совершила 67 ошибок. Модель-студент, будучи обученной с нуля без регуляризации, совершила 146 ошибок. Дистиллированная модель-студент совершила 74 ошибки. Как видим, подход действительно работает.
...
## Эффективная архитектура трансформера (автор: Звегинцев Максим)
...
### Multi query attention (автор: Хлопин Игорь)
...
### Grouped query attention (автор: Александр Гребенщиков)
...
## Инженерные подходы (автор: Хлопин Игорь)
