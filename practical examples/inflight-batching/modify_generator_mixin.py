...
def generate(
    ...
    **kwargs,
    ...
    # Comment by geaned: new criteria accounts for max_length and is incompatible with this one
...
def sample(
    ...
    **model_kwargs,
    ...
    if max_length is not None:
        ...
        stopping_criteria = GenerationMixin.validate_stopping_criteria(self, stopping_criteria, max_length)
    ...
    while True:
        ...
        if eos_token_id_tensor is not None:
            # Comment by geaned: eos_token is not the ultimate criterion of stopping the usage of a tensor row
            #                    because we also should we no queries waiting
            if not stopping_criteria[0].has_queries_left():
        ...
        # Comment by geaned: new criterion can mutate input_ids
        input_ids, is_done = stopping_criteria(input_ids, scores)
        if is_done:
            this_peer_finished = True
