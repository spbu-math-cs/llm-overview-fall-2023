...
* **zero-shot learning** – задача машинного обучения, подразумевающая проверку модели на новых, не известных ранее, данных. Конечно, для этого может потребоваться немного изменить промпты.
* **few-shot learning** – это задача машинного обучения, в которой модель надо преднастроить на тренировочном датасете так, чтобы она хорошо обучалась на ограниченном количестве новых размеченных примеров. То есть даже на новых классах или доменах данных мы сможем получить качественные предсказания. Фактически, это дообучение модели с правильно подобранными промптами на ограниченном количестве данных.
