...
SGD спасает от проблемы использования при этом $O(N*N*M*K)$ временной памяти и вычисления градиента за $O(N)$, заменяя $N$ на $B$. Он сходится более хаотично и медленно, но зато не требует таких огромных вычислительных мощностей. Однако, мы платим за это скоростью сходимости, получая $O(\frac{1}{\sqrt{N}})$, как доказано в [статье](https://arxiv.org/pdf/1606.04838.pdf).
