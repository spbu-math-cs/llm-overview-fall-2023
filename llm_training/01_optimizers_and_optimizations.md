...
* **Forward pass**: вычисляем выход модели по входу, подсчитываем значение функции потерь, при этом для каждого слоя запоминаем его вход (он же — выход предыдущего слоя; выход слоя также часто называется **активацией**).

    ![Forward pass](assets/forward_pass.png)

    На картинке представлен forward pass для некоторой нейронной сети, активации слоёв показаны в верхней части рисунка.

* **Backward pass**: подсчитываем градиенты функции потерь по входу/обучаемым весам слоёв, начиная с самого последнего слоя и двигаясь к самому первому. При этом для вычисления градиентов очередного слоя применяются градиенты следующих слоёв, чьим входом был выход данного слоя, и входы данного слоя.

    ![Backward pass](assets/backward_pass.gif)

    На анимации представлен backward pass для всё той же нейронной сети, вычисление градиентов проиллюстрировано в нижней её части. 
...
SGD спасает от проблемы использования при этом $O(N*N*M*K)$ временной памяти и вычисления градиента за $O(N)$, заменяя $N$ на $B$. К тому же, он вносит некоторый естественный шум, который помогает меньше переобучаться на данных и сходиться к лучшему результату, а не ближайшему локальному минимуму.
...
* предполагается, что тип данных, в котором хранятся числа, выполняются вычисления, занимает $N_{BYTES}$ байт в памяти. Так что для простоты записей необходимое домножение на константу будет опущено и будет применено только в самом конце подсчетов. А подробнее про типы использующихся чисел написано [далее](#mixed-precision-training-автор-георгий-ангени).

Первоначально для обучения нейронной сети все её веса необходимо разместить в памяти.
...
После первого шага обучения потребление памяти остается на том же уровне, поскольку не добавляется ничего нового и успешно переиспользуется ранее выделенная память (в предположении, что размер батча зафиксирован).

Таким образом, собирая всё вместе, с использованием Adam необходимо $\approx$ веса модели + все посчитанные активации + все градиенты весов + 2 * все градиенты весов = 4 * веса модели + все посчитанные активации памяти; в терминах байт — $N_{BYTES}$ * (4 * количество параметров модели + количество элементов в активациях).
...
Во-первых, использование gradient accumulation увеличивает время обучения в сравнении с применением увеличенного размера батча (как если бы сразу хватало памяти на размер батча $n \cdot m$).
...
Во-вторых, эквивалентность применения gradient accumulation и большого размера батча неверна для некоторых слоёв, опирающихся на статистики по батчу, например, для слоя Batch Normalization. Это поведение можно исправить, подкрутив параметр инерции `momentum` в конфигурации этого слоя. В качестве альтернативы использования этого слоя при gradient accumulation может стать применение Instance Normalization, Group Normalization. 
...
Предлагается посмотреть на то, в какой момент становится необходима активация при вычислении градиентов связанного слоя. В качестве примера рассмотрим шаг обучения полносвязной нейронной сети:
...
Выходы каких слоёв следует *чекпоинтить* в общем случае? Авторы оригинальной [статьи](https://arxiv.org/pdf/1604.06174.pdf), представившие метод, советуют не чекпоинтить слои, чьё вычисление не требует значительных затрат. Альтернативно, это означает то, что следует чекпоинтить слои, подсчёт которых является вычислительно дорогим, например, дороже (т.е. дольше в данном случае) чтения входа для этих слоёв.

 В статье приводится следующий пример: в связке слоёв "сверточный слой — нормализация — пулинг" лучше сохранить выход свёрточного слоя, а выходы остальных слоёв — отбросить, поскольку непосредственно их вычисление дешевле, чем применения сверточного слоя.

В случае больших языковых моделей следует сохранять выходы Attention слоёв, сочетающие в себе перемножение крупных матриц и применение Softmax (который подразумевает подсчёт тяжеловесной экспоненты), полносвязных сетей.
