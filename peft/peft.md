...
Представим: у вас есть задачи в области NLP, которые надо решить. Раньше лучшее, что вы могли сделать, — собрать и очистить много данных и с нуля обучить несколько моделей. Нет большого количества ресурсов — нет проекта.
...
Эти трудности решает **дообучение с эффективным использованием параметров (с англ. Parameter efficient fine-tuning, PEFT)**. PEFT-методы адаптируют предобученные модели к другим задачам без дообучения всех параметров и значительно сокращают затраты на вычисления и хранение. А еще устраняют риски катастрофического забывания и уменьшают размер контрольных точек до нескольких мегабайт.
...
Во время инференса, матрицы LoRA адаптеров складываются с весами модели: $\bar W \leftarrow W + \Delta W$. Это, по сравнению с обычными адаптерами, позволяет не создавать дополнительной задержки, но, с другой стороны, затрудняет обработку батчей с запросами по разным задачам.
